...
const UserForm = () => {
  ...
  const validate = () => {
    ...
    if (!/^\d{10}$/.test(user.phoneNumber)) tempErrors.phoneNumber = "Enter a valid 10-digit phone number";
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(user.email)) tempErrors.email = "Enter a valid Email ID";
